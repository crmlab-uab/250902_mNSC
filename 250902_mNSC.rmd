...
for (analysis_name in names(all_results_lists)) {
  ...
  for (comp_name in names(analysis_results)) {
    ...
    if (nrow(sig_genes) > 0) {
      print(reactable::reactable(sig_genes %>% 
              dplyr::arrange(padj) %>%
              dplyr::select(gene_name, human_ortholog, baseMean, log2FoldChange, padj, everything()), 
    ...
    cat("\n\n##### DE Kinase Box Plots\n")
    sig_kinases <- sig_genes %>% dplyr::filter(toupper(gene_name) %in% kinase_genes | toupper(human_ortholog) %in% kinase_genes)
