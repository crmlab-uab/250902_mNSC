...
```{r}
# Assign the specific subset to generic variables
samples_subset <- samples_ns1
files_sf_subset <- files_sf_ns1

# Source the child script, which will use the generic variables
source("./scripts/05_DESeq_NS1_subset.R", local = knitr::knit_global(), echo = TRUE)
```

### 6. Differential Expression Analysis (Host == 'BL6' Subset)
```{r}
# Re-assign the other specific subset to the same generic variables
samples_subset <- samples_bl6
files_sf_subset <- files_sf_bl6
