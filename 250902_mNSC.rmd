...
```{r setup-and-source, echo=TRUE, message=TRUE, warning=TRUE}
# First, load the 'here' package to resolve file paths correctly.
suppressPackageStartupMessages(library(here))

# Now, source all R scripts from the 'scripts' directory.
# This loads all other necessary packages and custom functions for the analysis.
# The script '00_setup.R' will load tidyverse, which provides the pipe '%>%' operator.
list.files(here("scripts"), pattern = "\\.R$", full.names = TRUE) |>
  purrr::walk(source)
```

## 1.2 Configure Project and Load Data
...
# It also creates the nested subdirectories for each analysis permutation.
create_project_dirs <- function(config) {
  # Create all top-level directories defined in the config
  dir_paths <- unlist(config[startsWith(names(config), "dir_")])
  sapply(dir_paths, dir.create, recursive = TRUE, showWarnings = FALSE)

  # Create all analysis-specific subdirectories for plots and CSVs
  for (s_name in names(config$analysis_definitions$sample_subsets)) {
    for (filt_name in names(config$analysis_definitions$gene_filters)) {
      for (m_type in config$analysis_definitions$model_types) {
        analysis_name <- sanitize_analysis_name(paste(s_name, filt_name, m_type, sep = "_"))
        dir.create(file.path(config$dir_graphs_png, analysis_name), recursive = TRUE, showWarnings = FALSE)
        dir.create(file.path(config$dir_graphs_pdf, analysis_name), recursive = TRUE, showWarnings = FALSE)
        dir.create(file.path(config$dir_data_csv, filt_name, analysis_name), recursive = TRUE, showWarnings = FALSE)
      }
    }
  }
  ...
  for (filt_name in names(cfg$analysis_definitions$gene_filters)) {
    ...
    for (m_type in cfg$analysis_definitions$model_types) {
      ...
      if (m_type == "additive") {
        ...
        if (length(all_terms) == 0 || length(design_terms) == 0) {
        ...
        main_terms <- intersect(cfg$deg_design_vars, relevant_vars)
      ...
      # Perform differential analysis, which includes extracting comparisons and generating plots
      all_results_lists[[analysis_name]] <- perform_differential_analysis(
        dds = dds_processed,
        comparisons = comparisons,
        gtf_map = gtf_maps$gene_name_map,
        p_adj_threshold = cfg$qval_threshold,
...
emit_gene_set_heatmap <- function(comp_label, gs_name, png_dir, date) {
  path <- file.path(png_dir, paste0(date, "_", comp_label, "_", gs_name, "_DEG_heatmap.png"))
  cat(paste0("\n##### ", gs_name, " Gene Set Heatmap\n"))
  if (file.exists(path)) {
    cat(paste0("![](", path, ")\n"))
  } else {
    cat("\n*No significant DEGs for this gene set, heatmap not generated.*\n")
  }
}

emit_gene_set_boxplots <- function(analysis_name, comp_label, gs_name, png_dir, date) {
  boxplot_dir <- file.path(png_dir, sanitize_analysis_name(file.path(analysis_name, paste0(comp_label, "_", gs_name, "_boxplots"))))
  cat(paste0("\n##### ", gs_name, " Gene Set Boxplots\n"))
  
  if (!dir.exists(boxplot_dir)) {
    cat("\n*No significant DEGs for this gene set, boxplots not generated.*\n")
    return()
  }
  
  files <- list.files(boxplot_dir, pattern = "\\.png$", full.names = TRUE)
  if (length(files) == 0) {
    cat("\n*No significant DEGs for this gene set, boxplots not generated.*\n")
  } else {
    for (f in files) emit_image(f)
  }
}

emit_standard_comp_plots <- function(comp_label, png_dir, date) {
  emit_labeled_image("Volcano Plot", file.path(png_dir, paste0(date, "_", comp_label, "_volcano.png")))
  emit_labeled_image("Top 50 DEG Heatmap", file.path(png_dir, paste0(date, "_", comp_label, "_DEG_heatmap_top50.png")))
  # Only show Top 250 if it exists
  path_250 <- file.path(png_dir, paste0(date, "_", comp_label, "_DEG_heatmap_top250.png"))
  if (file.exists(path_250)) {
    emit_labeled_image("Top 250 DEG Heatmap", path_250)
  }
}

emit_sig_gene_table <- function(sig_df, cfg) {
  if (nrow(sig_df) == 0) {
    cat("\n*No significant genes found for this comparison based on current thresholds.*\n")
...
for (analysis_name in names(all_dds_objects)) {
  ...
  emit_labeled_image("PCA", file.path(
    cfg$dir_graphs_png, safe_name,
    paste0(cfg$date, "_", analysis_name, "_PCA.png")
  ))
}

emit_section_header("\nSample Distance Heatmaps", level = 3)
for (analysis_name in names(all_dds_objects)) {
  safe_name <- sanitize_analysis_name(analysis_name)
  emit_section_header(gsub("_", " ", analysis_name), level = 4)
  emit_labeled_image("Distance Heatmap", file.path(
    cfg$dir_graphs_png, safe_name,
...
if (!is.null(cfg$qc_plot_settings$project_genes) &&
  ...
  for (analysis_name in names(all_dds_objects)) {
    ...
    plot_dir <- file.path(cfg$dir_graphs_png, safe_name)
...
for (analysis_name in names(all_results_lists)) {
  ...
  for (comp_full_name in names(analysis_results)) {
    comp_name <- sub(paste0(analysis_name, "_"), "", comp_full_name, fixed = TRUE)
    ...
    emit_standard_comp_plots(comp_name, png_dir, cfg$date) # Use the shorter name

    if (length(gene_sets) > 0) {
      for (gs_name in names(gene_sets)) {
        emit_gene_set_heatmap(comp_name, gs_name, png_dir, cfg$date)
        emit_gene_set_boxplots(analysis_name, comp_name, gs_name, png_dir, cfg$date)
      }
    }
  }
}
