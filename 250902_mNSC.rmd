...
RUN_STYLER <- TRUE
if (RUN_STYLER) {
  files_to_style <- c(here::here(paste0(cfg$project_name, ".Rmd")), list.files(here::here("scripts"), pattern = "\\.R$", full.names = TRUE))
...
set.seed(42)
# Function to find and create all directory paths in the config list
create_project_dirs <- function(config) {
  # Recursively find all values that look like directory paths
  dir_paths <- unlist(rapply(config, function(x) if (is.character(x) && grepl("dir_", deparse(substitute(x)))) x else NULL, how = "list"))
  # Add CSV output directories based on gene filters
...
for (s_name in names(cfg$analysis_definitions$sample_subsets)) {
  for (filt_name in names(cfg$analysis_definitions$gene_filters)) {
    ...
    gtf_maps <- create_tx2gene_maps(cfg$file_gtf, cfg$dir_input, filt_params$filter_gene_types)
    ...
    for (m_type in cfg$analysis_definitions$model_types) {
      ...
      dds_processed <- run_deseq_core(current_samples, current_files_sf, as.formula(design_str), gtf_maps$tx2gene_pcg, cfg$filter_by_factor, cfg$filter_min_count, parallel = (cfg$nc > 1))
      ...
      # <<< FIX: Pass the `filt_name` to the function
      all_results_lists[[analysis_name]] <- extract_comparisons(
        ...
        cfg = cfg, kinase_genes = kinase_genes, filt_name = filt_name
      )
    }
  }
}
save(cfg, samples, gtf_maps, all_dds_objects, all_results_lists, kinase_genes,
...
summary_list <- lapply(names(all_results_lists), function(analysis_name) {
  ...
  if (length(analysis_results) == 0) return(NULL)
...
for (analysis_name in names(all_results_lists)) {
  ...
  for (comp_name in names(analysis_results)) {
    ...
    path_kinase_heatmap <- here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", comp_name, "_Kinase_DEG_heatmap.png"))
    if (file.exists(path_kinase_heatmap)) cat(paste0("![](", path_kinase_heatmap, ")"))
    ...
    if (nrow(sig_kinases) > 0) {
      for (kinase_name in sig_kinases$gene_name) {
        plot_path <- here::here(cfg$dir_graphs_png_kinases, paste0(cfg$date, "_", comp_name, "_Boxplot_", kinase_name, ".png"))
