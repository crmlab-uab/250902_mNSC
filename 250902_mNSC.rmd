...
cfg <- list(
  ...
  analysis_definitions = list(
    # <<< FIX: Implemented requested sample subsetting plan >>>
    sample_subsets = list(
      Full_Dataset = list(),
...
if (RUN_ANALYSIS == FALSE) {
  ...
  if (file.exists(results_file_path)) {
    ...
    stop("RUN_ANALYSIS is FALSE, but no results file was found.")
  }
}
...
if (file.exists(cfg$file_kinases)) {
  ...
  # <<< FIX: Standardize kinase gene names to uppercase for matching >>>
  kinase_genes <- toupper(unique(kinase_list$Mouse_symbol))
} else {
  kinase_genes <- NULL
}
```
...
set.seed(42)
lapply(c(
  cfg$dir_output, cfg$dir_graphs_png, cfg$dir_graphs_pdf, cfg$dir_results_csv,
...
files_sf <- file.path(cfg$dir_sf, paste0(rownames(samples), ".sf")); names(files_sf) <- rownames(samples)

all_dds_objects <- list(); all_results_lists <- list()
...
for (s_name in names(cfg$analysis_definitions$sample_subsets)) {
  for (filt_name in names(cfg$analysis_definitions$gene_filters)) {
    ...
    gtf_maps <- create_tx2gene_maps(
      gtf_path = cfg$file_gtf, cache_dir = cfg$dir_input,
      filter_gene_types = filt_params$filter_gene_types,
      exclude_patterns = filt_params$exclude_patterns,
      exclude_chromosomes = filt_params$exclude_chromosomes
    )
    for (m_type in cfg$analysis_definitions$model_types) {
      ...
      if (length(subset_criteria) > 0) {
        current_samples <- droplevels(subset(samples, Reduce(`&`, lapply(names(subset_criteria), function(col) samples[[col]] %in% subset_criteria[[col]]))))
      }
      ...
      dds_processed <- run_deseq_core(current_samples, current_files_sf, design_formula, gtf_maps$tx2gene_pcg, cfg$filter_by_factor, cfg$filter_min_count)
      ...
      comparisons <- unlist(lapply(factors_to_compare, function(f) {
        if (length(levels(current_samples[[f]])) > 1) {
          combn(levels(current_samples[[f]]), 2, function(p) list(list(factor = f, group1 = p[1], group2 = p[2])), simplify = FALSE)
        }
      }), recursive = FALSE)
      
      all_results_lists[[analysis_name]] <- extract_comparisons(dds_processed, comparisons, analysis_name, gtf_maps$gene_name_map, cfg, kinase_genes)
    }
  }
}

save(cfg, samples, gtf_maps, all_dds_objects, all_results_lists, kinase_genes, file = here::here(cfg$dir_input, paste0(cfg$project_name, "_results.RData")))
```
...
for (analysis_name in names(all_dds_objects)) {
  cat(paste("\n### QC for", gsub("_", " ", analysis_name), "\n"))
  cat("\n#### PCA Plot\n")
  cat(paste0("![](", here::here(cfg$dir_graphs_png, paste0(analysis_name, "_PCA.png")), ")"))
  cat("\n#### Sample Distance Heatmap\n")
  cat(paste0("![](", here::here(cfg$dir_graphs_png, paste0(analysis_name, "_Heatmap.png")), ")"))
}
```
...
for (analysis_name in names(all_dds_objects)) {
  ...
  for (gene_name in cfg$qc_plot_settings$project_genes) {
    plot_path <- here::here(cfg$dir_graphs_png, paste0(analysis_name, "_GenePlot_", gene_name, ".png"))
...
summary_df <- do.call(rbind, lapply(names(all_results_lists), function(analysis_name) {
  do.call(rbind, lapply(names(all_results_lists[[analysis_name]]), function(comp_name) {
    res_df <- all_results_lists[[analysis_name]][[comp_name]]
    sig_df <- res_df %>% dplyr::filter(padj < cfg$qval_threshold & abs(log2FoldChange) > cfg$lfc_threshold)
    data.frame(Comparison = comp_name, `Total Genes` = nrow(res_df), DEGs = nrow(sig_df),
...
for (analysis_name in names(all_results_lists)) {
  ...
  for (comp_name in names(analysis_results)) {
    ...
    if (nrow(sig_genes) > 0) {
      print(reactable::reactable(sig_genes %>% arrange(padj), defaultPageSize = 5, searchable = TRUE, bordered = TRUE))
    } else { cat("\n*No significant genes found.*\n") }

    cat("\n\n##### Volcano Plot\n")
    cat(paste0("![](", here::here(cfg$dir_graphs_png, paste0(comp_name, "_volcano.png")), ")"))
    cat("\n\n##### Top 50 DEG Heatmap\n")
    cat(paste0("![](", here::here(cfg$dir_graphs_png, paste0(comp_name, "_DEG_heatmap_top50.png")), ")"))
    # <<< FIX: Embed the new Top 250 DEG heatmap >>>
    cat("\n\n##### Top 250 DEG Heatmap\n")
    path_250 <- here::here(cfg$dir_graphs_png, paste0(comp_name, "_DEG_heatmap_top250.png"))
    if(file.exists(path_250)) cat(paste0("![](", path_250, ")"))
    
    cat("\n\n##### Top Kinase DEG Heatmap\n")
    cat(paste0("![](", here::here(cfg$dir_graphs_png, paste0(comp_name, "_Kinase_DEG_heatmap.png")), ")"))

    cat("\n\n##### DE Kinase Box Plots\n")
    sig_kinases <- sig_genes %>% dplyr::filter(toupper(gene_name) %in% kinase_genes)
    if (nrow(sig_kinases) > 0) {
      for (kinase_name in sig_kinases$gene_name) {
        plot_path <- here::here(cfg$dir_graphs_png_kinase_boxplots, paste0(comp_name, "_Boxplot_", kinase_name, ".png"))
