...
# Load critical dependency to ensure function is found during knitting
library(tximport)
...
run_deseq_core <- function(samples_df,
  ...
  dds <- DESeqDataSetFromTximport(txi_data,
    colData = samples_df,
...
extract_comparisons <- function(dds_processed,
  ...
  for (comp in comparisons_list) {
    res_name <- paste0(
      analysis_name, "_", comp$factor, "_", comp$group1, "_vs_", comp$group2
    )
    ...
    csv_filename <- here::here(cfg$dir_results_csv, paste0(cfg$date, "_", res_name, "_results.csv"))
    write.csv(res_df, file = csv_filename, row.names = FALSE)

    sig_genes_df <- res_df %>% dplyr::filter(padj < cfg$qval_threshold & !is.na(padj))
    ...
    if (nrow(sig_genes_df) > 1) {
      message("......Generating Top 50 DEG heatmap.")
    ...
    if (!is.null(kinase_genes) && nrow(sig_genes_df) > 0) {
      ...
      if (nrow(sig_kinases_df) > 0) {
        message("......Generating Kinase DEG box plots.")
...
generate_deg_heatmap <- function(heatmap_df, vsd, dds, samples_in_comparison, cfg, res_name, plot_suffix, title) {
  ...
  annotation_col <- as.data.frame(colData(dds)[samples_in_comparison, c("Driver", "Model", "Host"), drop = FALSE])

  png_filename <- here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", res_name, "_", plot_suffix, ".png"))
  pdf_filename <- here::here(cfg$dir_graphs_pdf, paste0(cfg$date, "_", res_name, "_", plot_suffix, ".pdf"))
...
generate_kinase_boxplots <- function(sig_kinases_df, dds_processed, comp, cfg, res_name) {
  ...
  for (i in 1:nrow(sig_kinases_df)) {
    ...
    plot_filename_png <- here::here(cfg$dir_graphs_png_kinase_boxplots, paste0(cfg$date, "_", res_name, "_Boxplot_", gene_name, ".png"))
    plot_filename_pdf <- here::here(cfg$dir_graphs_pdf_kinase_boxplots, paste0(cfg$date, "_", res_name, "_Boxplot_", gene_name, ".pdf"))
