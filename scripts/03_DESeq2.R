...
run_deseq_core <- function(samples_df, files_sf, design_formula, tx2gene_map, filter_factor, min_count, parallel = FALSE) {
  txi <- tximport::tximport(files_sf, type = "salmon", tx2gene = tx2gene_map, ignoreTxVersion = TRUE)
...
#' @param comparisons_list A list of comparison definitions.
...
#' @param kinase_genes A character vector of kinase gene symbols for highlighting.
...
#'   for one comparison.
extract_comparisons <- function(dds_processed, comparisons_list, analysis_name, gene_map, cfg, kinase_genes, filt_name) {
  results_list <- list()
  ...
  for (comp in comparisons_list) {
    ...
    res <- DESeq2::results(dds_processed, contrast = c(factor, group2, group1), alpha = cfg$qval_threshold)
    ...
    file_path_csv <- here::here(csv_dir, paste0(cfg$date, "_", comp_name, "_DEG_results.csv"))
    write.csv(res_df, file_path_csv, row.names = FALSE)

    # Generate all plots for this specific comparison
    generate_comparison_plots(res_df, dds_processed, comp_name, cfg, kinase_genes, gene_map)
...
#'
#' Creates volcano plots, heatmaps, and kinase-specific plots for a given
...
#' @param kinase_genes A character vector of kinase gene symbols.
#' @param gene_map A data frame mapping gene IDs to gene names.
generate_comparison_plots <- function(res_df, dds, comp_name, cfg, kinase_genes, gene_map) {
  # Volcano Plot
  ...
  ggsave(here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", comp_name, "_volcano.png")), p_volcano, width = 10, height = 10)

  sig_genes <- res_df %>% dplyr::filter(padj < cfg$qval_threshold & abs(log2FoldChange) > cfg$lfc_threshold)
  if (nrow(sig_genes) < 2) {
    message(paste("...Skipping heatmaps for", comp_name, "due to < 2 significant genes."))
  ...
  p_heatmap50 <- pheatmap::pheatmap(norm_counts[top50_genes, ],
  ...
  ggsave(here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", comp_name, "_DEG_heatmap_top50.png")), p_heatmap50, width = 8, height = 12)

  # Top 250 DEG Heatmap (if applicable)
  if (nrow(sig_genes) > 50) {
    top250_genes <- head(sig_genes$gene_id, 250)
    p_heatmap250 <- pheatmap::pheatmap(norm_counts[top250_genes, ],
      annotation_col = as.data.frame(colData(dds)[, cfg$main_vars, drop = FALSE]),
    ...
    ggsave(here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", comp_name, "_DEG_heatmap_top250.png")), p_heatmap250, width = 8, height = 18)
  }

  # Kinase-specific analysis
  if (!is.null(kinase_genes)) {
    sig_kinases <- sig_genes %>% dplyr::filter(gene_name_upper %in% kinase_genes | toupper(human_ortholog) %in% kinase_genes)
    if (nrow(sig_kinases) > 1) {
      p_heatmap_kinase <- pheatmap::pheatmap(norm_counts[sig_kinases$gene_id, ],
        annotation_col = as.data.frame(colData(dds)[, cfg$main_vars, drop = FALSE]),
        labels_row = sig_kinases$gene_name,
        main = paste("DE Kinases:", gsub("_", " ", comp_name)),
        show_colnames = FALSE,
        scale = "row"
      )
      ggsave(here::here(cfg$dir_graphs_png, paste0(cfg$date, "_", comp_name, "_Kinase_DEG_heatmap.png")), p_heatmap_kinase, width = 8, height = 10)

      # Box plots for individual DE kinases
      for (i in 1:nrow(sig_kinases)) {
        gene_id <- sig_kinases$gene_id[i]
        gene_name <- sig_kinases$gene_name[i]
        plot_data <- DESeq2::plotCounts(dds, gene = gene_id, intgroup = cfg$main_vars, returnData = TRUE)
        p_box <- ggplot2::ggplot(plot_data, ggplot2::aes_string(x = "Driver", y = "count", fill = "Model")) +
          ggplot2::geom_boxplot(outlier.shape = NA) +
          ggplot2::geom_jitter(width = 0.2) +
          ggplot2::facet_wrap(~Host) +
          ggplot2::ggtitle(paste("Expression of", gene_name)) +
          ggplot2::theme_bw()
        ggsave(here::here(cfg$dir_graphs_png_kinases, paste0(cfg$date, "_", comp_name, "_Boxplot_", gene_name, ".png")), p_box, width = 8, height = 6)
      }
    }
  }
}